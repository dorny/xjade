...
ExpressionParts
  ...
  / SquareBrackets
  / CurlyBrackets
  / __ PrefixOperator __ Expression

ExpressionPartsIn
  = __ InfixOperator __ Expression
  / __ PostfixOperator
  / ExpressionParts

Expression
  = code:$(ExpressionParts ExpressionPartsIn*)
  { return {type: 'Code', value: code} }
...
Directive
  = "@" directive:(If / ElseIf / Else / For / Each / While / Switch / Case / UnknownDirective)
      {
...
If
  = "if" _* expr:ReqExpression
      { return {name: 'If', expr: expr.value}; }

ElseIf
  = "else" _* "if" _* expr:ReqExpression
      { return {name: 'ElseIf', expr: expr.value}; }
...
While
  = "while" _+ expr:ReqExpression
      { return {name:'While', expr:expr.value}; }

Switch
  = "switch" _+ expr:ReqExpression
    { return {name:'Switch', expr:expr.value}; }

Case
  = "case" _+ expr:ReqExpression
    { return {name:'Case', expr:expr.value}; }
